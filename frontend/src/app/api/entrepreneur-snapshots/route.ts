...
export async function GET(request: Request) {
  ...
  const isLatest = searchParams.get('is_latest');
  const pagination = parsePagination(searchParams);

  try {
    const supabase = await createServerSupabaseClient();
    const repo = createEntrepreneurRepository(supabase);
    const all = await repo.findAllSnapshots({
      entrepreneurId,
      isLatest: isLatest !== null ? isLatest === 'true' : undefined,
    });
    ...
    console.error('[entrepreneur-snapshots] Error:', err);
    return serverError();
  }
}
