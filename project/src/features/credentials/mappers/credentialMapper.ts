...
export function mapCredential(row: RawRow): Credential {
  return {
    ...
    issuerStellarAddress: (row.issuer_stellar_address as string) ?? null,
    issuerDid: (row.issuer_did as string) ?? null,
    subjectWalletId: (row.subject_wallet_id as string) ?? null,
    sponsorVaultId: (row.sponsor_vault_id as string) ?? null,
    onchainVcId: (row.onchain_vc_id as string) ?? null,
    onchainOwnerAddress: (row.onchain_owner_address as string) ?? null,
    onchainContractId: (row.onchain_contract_id as string) ?? null,
