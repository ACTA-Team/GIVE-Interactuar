...
export function getRpcUrl(network: StellarNetworkType): string {
  return STELLAR_NETWORK[network.toUpperCase() as Uppercase<StellarNetworkType>]?.rpcUrl ?? ''
}

export function getNetworkPassphrase(network: StellarNetworkType): string {
  return (
    STELLAR_NETWORK[network.toUpperCase() as Uppercase<StellarNetworkType>]?.networkPassphrase ?? ''
  )
}
